...
const register=async(req,res,next)=>{
    ...
    try {
        ...
        if(password.length < 6) res.status(500).json({message:"Parolanız en az 6 karakter uzunluğunda olmalı"})
        const passwordHash=await bcrypt.hash(password,12)
        if(!isEmail(email)){
            res.status(500).json({message:"Geçerli bir email adresi girin !!!"})  
        }
